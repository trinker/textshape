...
Most of the functions split, expand, or tidy a `vector`, `list`, `data.frame`, or `DocumentTermMatrix`.  The `combine`, `bind_list`, `duration`, & `mtabulate` functions are notable exceptions.  The table below describes the functions and their use:
...
The main shaping functions can be broken into the categories of (a) binding, (b) combining, &#40;c) tabulating, (d) spanning, (e) splitting, & tidying.  The majority of functions in **textshape** fall into the category of splitting and expanding (the semantic opposite of combining).  These sections will provide example uses of the functions from **textshape** within the three categories.
...
The `tidy_xxx` functions convert untidy structures into [tidy format](https://www.jstatsoft.org/article/view/v059i10).  Tidy formatted text data structures are particularly useful for itnerfacing with **ggplot2**, which expects this form.  
