...
Most of the functions split/expand a `vector`, `list` or `data.frame`.  The `combine` and `mtabulate` functions are notable exceptions.  The table below describes the functions and their use:
...
| `split_match`    | `vector`                       | Split vector at specified character/regex                    |
...
The main shaping functions can be broken into the categories of (a) combining, (b) tabulating, & &#40;c) Splitting.  The majority of functions in **textshape** fall into the last category of splitting and expanding (the semantic opposite of combining.  These sections will provide examples uses of the functions from **textshape** within thee categories.
...
Here I calculate the indices of every time the `vs` variable in the `mtcars` data set changes and then split the dataframe on those indices.
