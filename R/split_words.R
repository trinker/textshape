...
#' \code{split_words} tries to detect the text column with words.
...
#' ))
#' split_words(x)
#' split_words(x, lower=FALSE)
#'
#' data(DATA)
#' split_words(DATA)
#' split_words(DATA, lower=FALSE)
#'
#' ## Larger data set
#' split_words(hamlet)
split_words <- function(x, ...) {
    UseMethod("split_words")
}

#' @export
#' @rdname split_words
#' @method split_words default
split_words.default <- function(x, lower = TRUE, ...) {
    if (lower) {
...
#' @export
#' @rdname split_words
#' @method split_words data.frame
split_words.data.frame <- function(x, text.var = TRUE, lower = TRUE, ...) {
    ...
    z[, element_id := 1:.N]
    express1 <- parse(text=paste0(text.var, " := split_words.default(", text.var, ", lower = ", lower, ")"))
    z[, eval(express1)]
